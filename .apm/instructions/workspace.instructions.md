...
- `rslib.config.ts` 统一为 `@reforce/tooling-rslib` 的 `defineLibraryConfig()`（bundleless + tsgo dts + esm + node target 基线，见 `tooling/rslib/config.ts`）；包级差异通过 overrides 全量透传给 `mergeRslibConfig`（lib 数组按 `id: "esm"` 合并），不在各包重复基线配置。
