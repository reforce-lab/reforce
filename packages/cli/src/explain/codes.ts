...
const articles: Readonly<Record<string, DiagnosticArticle>> = {
  ...
  THROWS_WITHOUT_HANDLER: {
    ...
    article: [
      ...
      "",
      "Errors created with defineHttpError(...) are the exception: they carry their own status",
      "and code, the built-in dispatcher answers them with an RFC 9457 problem+json response,",
      "so @Throws accepts them without any registered handler.",
    ],
  },
  ...
  INVALID_ERROR_HANDLER_SIGNATURE: {
    ...
    article: [
      ...
      "",
      "@Throws additionally accepts a top-level const created with defineHttpError(...) from",
      "@reforce/web: those errors carry their own status and code, the built-in dispatcher",
      "answers them with problem+json, and no handler is needed (or possible — the factory",
      "returns a class expression no handle parameter can be annotated with).",
      "",
