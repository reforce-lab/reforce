...
async function requestGracefulShutdown(subprocess: ResultPromise): Promise<void> {
  if (process.platform === "win32") {
    ...
    if (sendMessage === undefined) {
      throw new Error("The Windows signal-event fixture has no IPC channel.");
    }
    await sendMessage.call(subprocess, { type: "reforce:e2e-signal", signal: "SIGBREAK" });
    return;
  }
  if (!subprocess.kill("SIGINT")) {
    throw new Error("Unable to deliver SIGINT to the development command fixture.");
  }
}
...
function applicationProjectTree(): FixtureTree {
  return {
...
function hmrApplicationSource(generation: "one" | "two"): string {
...
async function runScenario(input: {
  ...
  const subprocess = spawnDevCommandFixture([input.cwd, input.projectDirectory]);
...
describe("development command", () => {
  test("builds, starts, and gracefully closes a standalone application", async () => {
    const project = await createTemporaryProject(applicationProjectTree());
  ...
  test("reports a lease release failure as shutdown failure after graceful close", async () => {
    const project = await createTemporaryProject(applicationProjectTree());
    projects.push(project);
    const subprocess = spawnDevCommandFixture([project.projectRoot, ".", "", "fail-release"]);
    processes.push(subprocess);
  ...
  test("applies a real Node ESM hot update only after the previous Context closes", async () => {
    ...
    if (typeof src === "string" || src instanceof Uint8Array) {
      throw new Error("Expected an application source fixture directory.");
    }
    const project = await createTemporaryProject({
      ...tree,
      src: { ...src, "application.ts": hmrApplicationSource("one") },
    });
    projects.push(project);
    const subprocess = spawnDevCommandFixture([project.projectRoot, "."]);
    processes.push(subprocess);
    let stderr = "";
  ...
  test("keeps the healthy child and assets through a failed rebuild", async () => {
    ...
    await writeFile(join(project.projectRoot, "src", "application.ts"), "export class Broken {\n");
