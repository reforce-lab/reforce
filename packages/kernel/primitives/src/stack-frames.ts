...
// 仓库内自跑（packages/<分组>/<包>/dist）那一半按**框架包名单**认，不认任意
// `packages/*/*/dist`：用户自己的 monorepo 完全可能叫 packages/orders/dist，把用户帧折叠掉
// 正是「第三方帧常常是根因」要防的那种帮倒忙。名单是本仓运行期会出现在栈上的包的封闭集合；
// 新增运行期包时补一行，漏补的代价只是那个包的帧不折叠——偏吵，不偏静默。
//
// 写成 `<分组>/<包>` 整串而不是两个名单做叉乘，是因为叉乘会认出 kernel/web-node 这种并不存在
// 的组合，等于把名单放宽成「随便哪个分组下的框架包名」，白丢一层精度。
const frameworkDistDirectories = [
  "data/transaction",
  "devkit/cli",
  "kernel/config",
  "kernel/core",
...
const frameworkDistPattern = new RegExp(
  `${separator}packages${separator}(?:${frameworkDistDirectories
    .map((directory) => directory.split("/").join(separator))
    .join("|")})${separator}dist${separator}`,
  "u",
);
