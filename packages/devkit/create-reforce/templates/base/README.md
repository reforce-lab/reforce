...
通用的 404 / 401 / 403 / 409 不用你定义，`@reforce/web-core` 直接导出 `NotFoundError` 等五个，
`import` 就能抛。
...
- 逃生门：`REFORCE_DEV_ERROR_PAGE=off` 关掉错误页（只认 `off` 这一个值），dev 的错误响应
  回到与生产同形的脱敏 problem+json。服务缺省只监听 localhost，所以错误页平时只有你自己
  看得到；一旦按下面那条把 `WEB_SERVER_HOSTNAME` 配成 `0.0.0.0`，同一局域网里任何人访问一条
  出错路由就都能看到堆栈与源码——在不可信网络上这么做时请同时关掉错误页。
...
**监听地址**：缺省 `localhost`，只有本机连得上。手机戳局域网接口、或者部署进容器要让外面
连进来，配 `WEB_SERVER_HOSTNAME=0.0.0.0`（IPv6 用 `::`）。这条不给默认值是故意的：对外暴露
是个部署决定，得写出来，不该由「没配置」得到。
...
**加一种异常**：通用的直接用 `@reforce/web-core` 的 `NotFoundError` / `UnauthorizedError` /
