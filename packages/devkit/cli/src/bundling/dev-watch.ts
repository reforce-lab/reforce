...
// 所以 dev 构建把它整个外置。构建期从用户项目 resolve @reforce/web-core、再从 web 的真实位置
// resolve 渲染器——pnpm 严格布局下它只对 @reforce/web-core 可见，对用户项目不可见。产物里落成
// `import … from "file://…"`，运行时从真实 node_modules 加载，资产路径天然正确。
//
// 解析不到就不外置（undefined）：没装 @reforce/web-core 的项目不可能命中渲染路径；万一 web 在而
...
function resolveDevErrorRendererUrl(projectRoot: string): string | undefined {
  try {
    ...
    const webEntryPath = projectRequire.resolve("@reforce/web-core");
