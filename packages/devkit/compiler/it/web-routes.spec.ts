  ...
  test("a compiled application serves requests through scope, chain, slot decoding, and error handling", async () => {
    const { project, result } = await compileTree(
      {
        ...
        src: {
          ...
          "trace.ts": [
            'import { Injectable } from "@reforce/core";',
            'import { Middleware, type RequestContext, type RouteResponse } from "@reforce/web-core";',
            'import { LogBook } from "@/log-book";',
            '@Middleware({ phase: "observability", global: true })',
            "export class TraceMiddleware {",
            "  constructor(private readonly log: LogBook) {}",
            "  async handle(context: RequestContext, next: () => Promise<RouteResponse>): Promise<RouteResponse> {",
          ...
          "auth.ts": [
            'import { Injectable } from "@reforce/core";',
            'import { Middleware, type RequestContext, type RouteOutcome, type RouteResponse } from "@reforce/web-core";',
            'import { LogBook } from "@/log-book";',
            'import { Roles } from "@/markers";',
            '@Middleware({ phase: "admission" })',
            "export class AuthMiddleware {",
            "  constructor(private readonly log: LogBook) {}",
            "  handle(context: RequestContext, next: () => Promise<RouteResponse>): RouteOutcome | Promise<RouteOutcome> {",
    ...
    await writeFile(
      ...
      [
        ...
        "    }",
        "    // 自建适配器手里已经有标准 Request，用 fromStandardRequest 接进契约（#341）；",
        "    // 框架自己的引擎反过来，整个价值就是不造这个对象。",
        ...
        '      instance: new RequestHolder(context.request.headers.get("x-user") ?? "anonymous"),',
        "    },",
        "  ],",
        "});",
        ...
        '  { id: "42" },',
        ");",
        "const okBody = await readRouteBody(ok);",
        ...
        "  {},",
        ");",
        "const explodedBody = await readRouteBody(exploded);",
