  ...
  const stdout = await compileAndRun(
    {
      ...
      "interceptors.ts": [
        ...
        "  async intercept<R>(context: MethodInvocationContext<{ detail: boolean } | undefined>, next: () => Promise<R>): Promise<R> {",
        ...
        "  async intercept<R>(context: MethodInvocationContext<{ label: string }>, next: () => Promise<R>): Promise<R> {",
  ...
  const stdout = await compileAndRun(
    {
      ...
      "orders.ts": [
        ...
        "@Injectable()",
        "export class Orders {",
        "  constructor(private readonly manager: RecordingManager) {}",
        "",
        "  @Transactional()",
        "  async save(): Promise<string> {",
        "    events.push(`inside:${this.manager.current()}`);",
        '    return "saved";',
        "  }",
        "",
