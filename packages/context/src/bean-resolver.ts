...
export class BeanResolver {
  ...
  #cycleProxy(targetId: string): object {
  ...
  #resolveProxyTarget(targetId: string): object {
  ...
  #lazy(targetId: string): Lazy<object> {
  ...
  #resolveLazyTarget(targetId: string): object {
    const record = this.#state.record(targetId);
