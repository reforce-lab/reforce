...
function applicationTree(packageName: string, packageTree?: ProjectTree): ProjectTree {
  return {
...
function declarationPackage(files: ProjectTree): ProjectTree {
  return {
...
function sourceTree(source: string, additional: ProjectTree = {}): ProjectTree {
  return {
...
async function compileSource(source: string, additional?: ProjectTree): Promise<CompileResult> {
...
describe("module resolution watch inputs", () => {
  test("does not watch ancestor module directories for runtime builtins", async () => {
    ...
    const ancestorModuleDirectories: string[] = [];
    let current = path.dirname(project.projectRoot);
    for (;;) {
      ancestorModuleDirectories.push(path.join(current, "node_modules"));
    ...
    expect(result.status).toBe("success");
    expect(
      result.watchInputs.missingDependencies.filter((dependency) =>
        ancestorModuleDirectories.includes(dependency),
      ),
    ).toEqual([]);
  });

  test("excludes every strict ancestor of the project root from directory watches", async () => {
    const { project, result } = await compile(
      applicationTree(
        "external-contract",
        declarationPackage({ "index.d.ts": "export interface Port {}\n" }),
    ...
    const strictAncestors: string[] = [];
    let current = path.dirname(project.projectRoot);
    for (;;) {
      strictAncestors.push(current);
      const parent = path.dirname(current);
      if (parent === current) {
        break;
      }
    ...
    expect(result.watchInputs.contextDependencies).toContain(project.projectRoot);
    for (const ancestor of strictAncestors) {
      expect(result.watchInputs.contextDependencies).not.toContain(ancestor);
    }
  });
...
describe("project linking", () => {
  test("links an interface imported through a namespace export", async () => {
    const input = await copiedProject("namespace-export-contract");
    const compiler = createCompiler();
  ...
  test("resolves a standard package import from the application manifest", async () => {
    const { project, result } = await compile({
      "package.json": `${JSON.stringify({
        name: "package-import-application",
        private: true,
        type: "module",
        imports: {
          "#package-import": {
            types: "./src/package-import.ts",
            default: "./src/package-import.ts",
          },
        },
      })}\n`,
      "tsconfig.json": applicationTsconfig(),
      src: {
        ...
        "package-import.ts": "export interface PackageImportPort {}\n",
      },
    });
    ...
    if (result.status === "success") {
      ...
      expect(manifest).toContain('"moduleSpecifier": "../../src/package-import.js"');
    }
  });

  test("links package exports, subpath exports, and paths without discovering shared Beans", async () => {
    const input = await copiedProject("monorepo-application-selection");
    const target = path.join(input.projectRoot, "node_modules", "@fixture", "shared");
    await mkdir(path.dirname(target), { recursive: true });
    await symlink(
      path.join(input.projectRoot, "packages", "shared"),
      target,
    ...
    const manifest = result.files.find((file) => file.path === "manifest.json")?.content;
    const canonicalMonorepoRoot = await realpath(input.projectRoot);
    const logicalPackageDirectory = path.join(
      canonicalMonorepoRoot,
      "node_modules",
      "@fixture",
      "shared",
    );
    const physicalPackageDirectory = await realpath(
      path.join(input.projectRoot, "packages", "shared"),
    );
    const strictAncestors = new Set<string>();
    for (const applicationRoot of [project.projectRoot, appRoot]) {
      let current = path.dirname(applicationRoot);
      for (;;) {
        strictAncestors.add(current);
        const parent = path.dirname(current);
        if (parent === current) {
          break;
        }
