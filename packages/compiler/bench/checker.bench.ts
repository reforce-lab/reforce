...
// 复跑:`pnpm --dir packages/compiler run bench:checker`。
//
// 三组:
//   A 冷启动:spawn + 首个 updateSnapshot + 代表性查询负载,对照同项目 tsgo `tsc -p` 全量
//     检查基线(增幅判据 ≤1×);500/2000 文件合成项目 + 仓库自身 packages/compiler。
...
}
{
  ...
  const t1 = performance.now();
  console.log(
    `self(packages/compiler) | tsc-baseline=${formatMs(baseline)} | spawn+snapshot=${formatMs(t1 - t0)}`,
  );
  api.close();
}
