...
import { copyCompilerFixture, resolveProjectOrThrow } from "./support/compiler-fixture";
...
describe("project resolution", () => {
  ...
  test("does not descend from a solution config into referenced applications", async () => {
    ...
    const result = await compiler.resolveProject({ projectDirectory: fixture.projectRoot });
  ...
  test("canonicalizes symlink, parent-segment, and case-only aliases", async () => {
    ...
    const fixture = await copiedFixture("standalone-application");
    await writeFixtureTree(applicationDirectory, {
      "tsconfig.json": await readFile(path.join(fixture.projectRoot, "tsconfig.json"), "utf8"),
      src: {
        "application.ts": await readFile(
