...
beforeAll(async () => {
  ...
  await copyFixtureTree(
    path.join(fixtureDirectory, "provider-selection-runtime", "project"),
    project.projectRoot,
  );
...
describe("provider selection", () => {
  test("a unique interface candidate is selected", () => {
    expect(dependencyTarget(validSelection, "src/application.ts#SelectionProbe", 2)).toBe(
      "src/application.ts#UniqueProvider",
    );
  });

  test("one Primary is selected as the unqualified interface default", () => {
    expect(dependencyTarget(validSelection, "src/application.ts#SelectionProbe", 0)).toBe(
      "src/application.ts#PreferredProvider",
    );
  });

  test("a qualifier selects its exact provider without consulting Primary", () => {
    expect(dependencyTarget(validSelection, "src/application.ts#SelectionProbe", 1)).toBe(
