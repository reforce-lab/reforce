...
// 请求字段贡献者（RFC 0011 L4，#242 影响面点名的「@reforce/web：请求字段的 LogFieldSource
// 实现」）。
//
...
//
// **为什么 @reforce/web 自己拿一个 ALS**：贡献者要进 LoggerFactory 的集合注入，而工厂是单例、
 ...
 * @reforce/logging 变成 @reforce/web 的硬依赖——同 RequestLogger 的理由。
 *
 * 它没有 `@Injectable()`：@reforce/web 不是 starter（没有 reforce-meta.json），本包的任何类都
