...
//
// **这张表的噪声下限是 10%~20%，不是个位数。** 实测同一份代码连跑两次，裸基线几乎不动而
// Reforce 侧差了 21%，比值从 23.7% 漂到 18.9%。所以每个测量点跑 defaultRepetitions 轮取中位数，
// 并把极差作为 spread 印在表里：**比 spread 小的改动，单看这张表得不出结论，必须回到微基准。**
...
export interface LoadResult {
  ...
  /**
   * 本测量点各轮吞吐的极差占中位数的比例。它是**这次测量的噪声下限**：小于它的改动，单看
   * 这张表得不出结论，得回到微基准。实测同一目标跑两次可以差 10%~20%，所以这个数字必须印
   * 出来——否则读者会把噪声当成收益。
   */
