...
import type { RequestContext } from "@reforce/web-core";

// 根请求 bean（ADR 0006 W7）：值 = 本次请求的 RequestContext，由 webRequestSeeder（见
...
export class RequestAudit {
  ...
  get requestId(): string {
    return this.exchange.context.request.headers.get("x-request-id") ?? "anonymous";
  }

  get matchedPath(): string {
    return this.exchange.context.path;
  }
}
