...
const temporaryRoot = fileURLToPath(new URL("..", import.meta.url));
const projectRoot = join(temporaryRoot, "project");
const bootstrapModule: unknown = await import(
  pathToFileURL(join(projectRoot, ".reforce", "generated", "bootstrap.ts")).href
);
const applicationModule: unknown = await import(
  pathToFileURL(join(projectRoot, "src", "application.ts")).href
);
...
const runtimeObservation = {
  ...
  alphaMarker: requireNumber(Reflect.get(alpha, "marker"), "AlphaService marker must be a number."),
