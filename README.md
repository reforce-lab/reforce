...
├── tooling/
│   └── tsconfig/          # @reforce/tooling-tsconfig —— 共享 TS 配置
...
```bash
bun install          # 安装依赖（自动安装 git hooks）
bun run --filter @reforce/playground dev   # 跑 playground 沙箱（watch 模式）
bun run check:write  # Biome 检查并自动修复（turbo 调度各子包）
bun run typecheck    # TS7 全仓类型检查
bun run test         # bun test（内建 runner）
bun run build        # 构建所有包
```
